...
LLVMMetadataRef LLVMDIBuilderCreateCompileUnit(LLVMDIBuilderRef Dref,
  ...
  DIBuilder *D = unwrap(Dref);
  return wrap(D->createCompileUnit(Lang, D->createFile(File, Dir), Producer, Optimized, Flags,
                                   RuntimeVersion));
}
...
LLVMMetadataRef LLVMDIBuilderCreateFunction(
  ...
  return wrap(D->createFunction(unwrap<DIScope>(Scope), Name, LinkageName,
                                ...
                                IsLocalToUnit, IsDefinition, ScopeLine, static_cast<DINode::DIFlags>(Flags),
                                IsOptimized));
}
...
LLVMMetadataRef LLVMDIBuilderCreateClassType(
    ...
    DIBuilder *D = unwrap(Dref);
    return wrap(D->createClassType(unwrap<DIScope>(Scope), Name, File ? unwrap<DIFile>(File) : nullptr, Line,
                            SizeInBits, AlignInBits, /*OffsetInBits*/ 0, static_cast<DINode::DIFlags>(Flags),
...
LLVMMetadataRef LLVMDIBuilderCreateReplaceableCompositeType(
  ...
  return wrap(D->createReplaceableCompositeType(
      ...
      Line, RuntimeLang, SizeInBits, AlignInBits, static_cast<DINode::DIFlags>(Flags)));
}
...
LLVMDIBuilderCreateMemberType(LLVMDIBuilderRef Dref, LLVMMetadataRef Scope,
  ...
  return wrap(D->createMemberType(
      unwrap<DIScope>(Scope), Name, File ? unwrap<DIFile>(File) : nullptr, Line,
      SizeInBits, AlignInBits, OffsetInBits, static_cast<DINode::DIFlags>(Flags), unwrap<DIType>(Ty)));
}
...
LLVMValueRef LLVMDIBuilderInsertDeclareAtEnd(LLVMDIBuilderRef Dref,
  ...
  DIBuilder *D = unwrap(Dref);
  DebugLoc loc = DebugLoc::get(Line, Col, Scope ? unwrap<MDNode>(Scope) : nullptr,
                  InlinedAt ? unwrap<MDNode>(InlinedAt) : nullptr);

  Instruction *Instr = D->insertDeclare(
      unwrap(Storage), unwrap<DILocalVariable>(VarInfo),
      unwrap<DIExpression>(Expr), /* DebugLoc */ loc, unwrap(Block));
  return wrap(Instr);
}
